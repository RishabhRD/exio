 ...
 * Copyright (c) 2024 Rishabh Dwivedi
 *
...
// Taken from stdexec and modified for use
...
#include <stdexec/exec/__detail/__atomic_ref.hpp>
#include <stdexec/exec/__detail/__bit_cast.hpp>

#include <stdexec/exec/linux/memory_mapped_region.hpp>
#include <stdexec/exec/linux/safe_file_descriptor.hpp>

#include <stdexec/exec/scope.hpp>
...
inline auto __io_uring_enter(int __ring_fd, unsigned int __to_submit,
                             unsigned int __min_complete, unsigned int __flags)
    -> int {
  int rc =
      static_cast<int>(::syscall(__NR_io_uring_enter, __ring_fd, __to_submit,
                                 __min_complete, __flags, nullptr, 0));
  if (rc == -1) {
    return -errno;
  } else {
    return rc;
...
inline auto __map_region(int __fd, ::off_t __offset, std::size_t __size)
  ...
  void *__ptr = ::mmap(nullptr, __size, PROT_READ | PROT_WRITE,
                       MAP_SHARED | MAP_POPULATE, __fd, __offset);
...
struct __context_base : stdexec::__immovable {
  explicit __context_base(unsigned __entries, unsigned __flags = 0)
    ...
    __submission_queue_region_ =
        __map_region(__ring_fd_, IORING_OFF_SQ_RING, __sring_sz);
    __submission_queue_entries_ =
        __map_region(__ring_fd_, IORING_OFF_SQES, __sqes_sz);
    if (!(__params_.features & IORING_FEAT_SINGLE_MMAP)) {
      __completion_queue_region_ =
          __map_region(__ring_fd_, IORING_OFF_CQ_RING, __cring_sz);
    }
  }
...
struct __task : stdexec::__immovable {
  ...
  explicit __task(const __task_vtable &__vtable) : __vtable_{&__vtable} {}
};
...
public:
  explicit __submission_queue(const memory_mapped_region &__region,
                              const memory_mapped_region &__sqes_region,
                              const ::io_uring_params &__params)
      : __head_{*__at_offset_as<__u32 *>(__region.data(),
                                         __params.sq_off.head)},
        __tail_{
            *__at_offset_as<__u32 *>(__region.data(), __params.sq_off.tail)},
        __array_{
            __at_offset_as<__u32 *>(__region.data(), __params.sq_off.array)},
        __entries_{static_cast<::io_uring_sqe *>(__sqes_region.data())},
        __mask_{*__at_offset_as<__u32 *>(__region.data(),
                                         __params.sq_off.ring_mask)},
  ...
  // If is_stopped is true, no new tasks are submitted to the io_uring unless it
  // is a cancellation. If is_stopped is true and a task is not ready to be
  // completed, the task is completed with an io_uring_cqe object with the
  // result field set to -ECANCELED.
  auto submit(__task_queue __tasks, __u32 __max_submissions,
              bool __is_stopped) noexcept -> __submission_result {
    ...
    __max_submissions =
        std::min(__max_submissions, __n_total_slots_ - __current_count);
...
public:
  explicit __completion_queue(const memory_mapped_region &__region,
                              const ::io_uring_params &__params) noexcept
      : __head_{*__at_offset_as<__u32 *>(__region.data(),
                                         __params.cq_off.head)},
        __tail_{
            *__at_offset_as<__u32 *>(__region.data(), __params.cq_off.tail)},
        __entries_{__at_offset_as<::io_uring_cqe *>(__region.data(),
                                                    __params.cq_off.cqes)},
        __mask_{*__at_offset_as<__u32 *>(__region.data(),
                                         __params.cq_off.ring_mask)} {}

  // This function first completes all tasks that are ready in the completion
  // queue of the io_uring. Then it completes all tasks that are ready in the
  // given queue of ready tasks. The function returns the number of previously
  // submitted completed tasks.
  auto complete(stdexec::__intrusive_queue<&__task::__next_> __ready =
                    __task_queue{}) noexcept -> int {
...
struct __wakeup_operation : __task {
  ...
  static auto __ready_(__task *) noexcept -> bool { return false; }
  ...
  __wakeup_operation(context_t *__ctx, int __eventfd)
      : __task{__vtable}, __context_{__ctx}, __eventfd_{__eventfd} {}
...
enum class until { stopped, empty };

class context_t : __context_base {
public:
  explicit context_t(unsigned __entries = 1024, unsigned __flags = 0)
      : __context_base(std::max(__entries, 2u), __flags),
        __completion_queue_{__completion_queue_region_
                                ? __completion_queue_region_
                                : __submission_queue_region_,
                            __params_},
        __submission_queue_{__submission_queue_region_,
                            __submission_queue_entries_, __params_},
        __wakeup_operation_{this, __eventfd_} {}

  void wakeup() {
    std::uint64_t __wakeup = 1;
    __throw_error_code_if(
        ::write(__eventfd_, &__wakeup, sizeof(__wakeup)) == -1, errno);
  }

  /// @brief Resets the io context to its initial state.
  void reset() {
    if (__is_running_.load(std::memory_order_relaxed) ||
        __n_total_submitted_ > 0) {
      throw std::runtime_error(
          "exio::io_uring_context::reset() called on a running context");
    }
  ...
  /// @brief  Breaks out of the run loop of the io context without stopping the
  /// context.
  void finish() {
  ...
  /// \returns true if the task was submitted, false if this io context and this
  /// task is have been stopped.
  auto submit(__task *__op) noexcept -> bool {
    // As long as the number of in-flight submissions is not
    // __no_new_submissions, we can increment the counter and push the operation
    // onto the queue. If the number of in-flight submissions is
    // __no_new_submissions, we have already finished the stop operation of the
    // io context and we can immediately stop the operation inline. Remark: As
    // long as the stopping is in progress we can still submit new operations.
    // But no operation will be submitted to io uring unless it is a
    // cancellation operation.
    int __n = 0;
    while (
        __n != __no_new_submissions &&
  ...
  ///
  /// This function is not thread-safe and must only be called from the thread
  /// that drives the io context.
  void run_some() noexcept {
    ...
    STDEXEC_ASSERT(0 <= __n_total_submitted_ &&
                   __n_total_submitted_ <=
                       static_cast<std::ptrdiff_t>(__params_.cq_entries));
  ...
  void run_until_stopped() {
    ...
    if (!__is_running_.compare_exchange_strong(expected_running, true,
                                               std::memory_order_relaxed)) {
      throw std::runtime_error(
          "exio::io_uring_context::run() called on a running context");
    } else {
      // Check whether we restart the context after a context-wide stop.
      // We have to reset the stop source in this case.
      int __in_flight =
          __n_submissions_in_flight_.load(std::memory_order_relaxed);
      if (__in_flight == __no_new_submissions) {
        __stop_source_.emplace();
        // Make emplacement of stop source visible to other threads and open the
        // door for new submissions.
    ...
    while (__n_total_submitted_ > 0 || !__pending_.empty()) {
      ...
      if (__n_total_submitted_ == 0 ||
          (__n_total_submitted_ == 1 &&
           __break_loop_.load(std::memory_order_acquire))) {
      ...
      int rc = __io_uring_enter(__ring_fd_,
                                ...
                                __min_complete, IORING_ENTER_GETEVENTS);
    ...
    if (__stop_source_->stop_requested() && __pending_.empty()) {
      ...
      while (!__n_submissions_in_flight_.compare_exchange_weak(
          __n_in_flight_expected, __no_new_submissions,
          std::memory_order_relaxed)) {
      ...
      STDEXEC_ASSERT(__n_submissions_in_flight_.load(
                         std::memory_order_relaxed) == __no_new_submissions);
  ...
  struct __on_stop {
    ...
    void operator()() const noexcept { __context_.request_stop(); }
  };

  template <class _Rcvr> struct __run_op {
    using __id = __run_op;
    using __t = __run_op;
    _Rcvr __rcvr_;
    context_t &__context_;
  ...
  auto get_scheduler() noexcept -> scheduler_t;

private:
  friend struct __wakeup_operation;

  // This constant is used for __n_submissions_in_flight to indicate that no new
  // submissions to this context will be completed by this context.
...
concept __io_task = //
    requires(_Op &__op, ::io_uring_sqe &__sqe, const ::io_uring_cqe &__cqe) {
      { __op.context() } noexcept -> std::convertible_to<context_t &>;
...
concept __stoppable_task = //
    ...
    requires(_Op &__op) {
      ...
      } noexcept -> stdexec::receiver_of<
          stdexec::completion_signatures<stdexec::set_stopped_t()>>;
    };

template <__stoppable_task _Op>
using __receiver_of_t =
    stdexec::__decay_t<decltype(std::declval<_Op &>().receiver())>;

template <__io_task _Base> struct __io_task_facade : __task {
  ...
  static void __complete_(__task *__pointer,
                          const ::io_uring_cqe &__cqe) noexcept {
    auto *__self = static_cast<__io_task_facade *>(__pointer);
  ...
  template <class... _Args>
    requires stdexec::constructible_from<_Base, std::in_place_t, __task *,
                                         _Args...>
  __io_task_facade(std::in_place_t, _Args &&...__args) //
      noexcept(stdexec::__nothrow_constructible_from<_Base, __task *, _Args...>)
      : __task{__vtable}, __base_(std::in_place, static_cast<__task *>(this),
  ...
  __io_task_facade(std::in_place_t, _Args &&...__args) //
      ...
      : __task{__vtable}, __base_(static_cast<_Args &&>(__args)...) {}

  auto base() noexcept -> _Base & { return __base_; }
...
template <class _Base> struct __stop_operation {
  ...
  public:
...
#ifdef STDEXEC_HAS_IO_URING_ASYNC_CANCELLATION
      if constexpr (requires(_Base *__op, ::io_uring_sqe &__sqe_) {
                      __op->submit_stop(__sqe_);
                    }) {
    ...
    static constexpr __task_vtable __vtable{&__ready_, &__submit_,
                                            &__complete_};

    explicit __t(_Base *__op) noexcept : __task(__vtable), __op_{__op} {}

    void start() noexcept {
      int expected = 1;
      if (__op_->__n_ops_.compare_exchange_strong(expected, 2,
                                                  std::memory_order_relaxed)) {
...
template <class _Base, bool _False> struct __impl_base {
  __task *__parent_;
  _Base __base_;
  ...
  __impl_base(__task *__parent, std::in_place_t, _Args &&...__args) //
      noexcept(stdexec::__nothrow_constructible_from<_Base, _Args...>)
      : __parent_{__parent}, __base_(static_cast<_Args &&>(__args)...) {}
};

template <class _Base> struct __impl_base<_Base, true> {
  __task *__parent_;
  _Base __base_;
...
template <__stoppable_task _Base> struct __stoppable_task_facade {
  using _Receiver = __receiver_of_t<_Base>;

  template <class _Ty>
  static constexpr bool __has_submit_stop_v = requires(
      _Ty &__base, ::io_uring_sqe &__sqe) { __base.submit_stop(__sqe); };
  ...
  struct __impl : __base_t {
    struct __stop_callback {
      ...
      void operator()() noexcept { __self_->__stop_operation_.start(); }
    };

    using __on_context_stop_t =
        std::optional<stdexec::inplace_stop_callback<__stop_callback>>;
    using __on_receiver_stop_t =
        std::optional<typename stdexec::stop_token_of_t<stdexec::env_of_t<
            _Receiver> &>::template callback_type<__stop_callback>>;
    ...
    auto context() noexcept -> context_t & { return this->__base_.context(); }
    ...
    void submit(::io_uring_sqe &__sqe) noexcept {
      [[maybe_unused]] int prev =
          __n_ops_.fetch_add(1, std::memory_order_relaxed);
      STDEXEC_ASSERT(prev == 0);
      context_t &__context_ = this->__base_.context();
      _Receiver &__receiver = this->__base_.receiver();
      __on_context_stop_.emplace(__context_.get_stop_token(),
                                 __stop_callback{this});
      __on_receiver_stop_.emplace(
          stdexec::get_stop_token(stdexec::get_env(__receiver)),
          __stop_callback{this});
...
template <class _Receiver> struct __stoppable_op_base {
  context_t &__context_;
  _Receiver __receiver_;

  auto receiver() & noexcept -> _Receiver & { return __receiver_; }

  auto receiver() && noexcept -> _Receiver && {
    return static_cast<_Receiver &&>(__receiver_);
  }

  auto context() noexcept -> context_t & { return __context_; }
};
