...
template <typename ReceiverId> struct async_write_some_operation {
  ...
  struct impl : public __stoppable_op_base<Receiver> {
    ...
    impl(context_t &ctx_, int fd_, std::span<std::byte const> buffer_,
...
template <typename Scheduler> struct async_write_some_sender {
  int fd;
  std::span<std::byte const> buffer;
  ...
  auto connect(Receiver &&receiver) const
      -> stdexec::__t<async_write_some_operation<stdexec::__id<Receiver>>> {
    return stdexec::__t<async_write_some_operation<stdexec::__id<Receiver>>>(
